import { emojis, noop, resourcesOrder } from "../../core/constants";
...
export default class MatchRenderer {
  ...
  constructor(
    ...
    this.mapRenderer = mapRendererFactory(mapName, svg, 10, 10, 1);
  }
  ...
  drawPlayers(players: Player[]) {
    ...
    players.forEach((player) => {
      ...
      if (player.quantityToDiscard > 0) {
        ...
        discarding.textContent = emojis.misc.discarding;
    ...
    resourcesOrder.forEach((resource) => {
      if (hand[resource] > 0) {
        for (let i = 0; i < hand[resource]; ++i) {
          ...
          text.textContent = emojis.resources[resource];
    ...
    devTypes.forEach((type) => {
      if (devHand[type] > 0) {
        for (let i = 0; i < devHand[type]; ++i) {
          ...
          const text = document.createElement("span");
          text.textContent = emojis.devCards[type];
          element.appendChild(text);
